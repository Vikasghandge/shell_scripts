...
import_from_config_file() {
    ...
    log_message "SUCCESS" "✅ Imported $loaded_count credentials from $creds_file"
    return 0
}
...
import_from_encrypted_file() {
    ...
    log_message "INFO" "🔓 Decrypting credentials file..."
    ...
    if [[ $? -eq 0 && -n "$decrypted_content" ]]; then
        ...
        log_message "SUCCESS" "✅ Imported $loaded_count encrypted credentials"
        return 0
    else
        log_message "ERROR" "❌ Failed to decrypt credentials file"
        return 1
    fi
}
...
import_from_vault() {
    ...
    log_message "INFO" "🏦 Importing credentials from HashiCorp Vault..."
        ...
        for server_ip in "${!SERVERS[@]}"; do
            ...
            if [[ $? -eq 0 ]]; then
                ...
                if [[ "$username" != "null" && "$password" != "null" ]]; then
                    ...
                    log_message "SUCCESS" "✅ Imported credentials for $server_ip from Vault"
                fi
            else
                log_message "WARNING" "⚠️  Could not retrieve credentials for $server_ip from Vault"
            fi
        done
    else
        log_message "ERROR" "❌ Vault CLI not available. Install from: https://developer.hashicorp.com/vault/downloads"
        return 1
    fi
}
...
import_from_aws_secrets() {
    ...
    log_message "INFO" "☁️  Importing credentials from AWS Secrets Manager..."
    ...
    if [[ $? -eq 0 && -n "$secret_value" ]]; then
        ...
        for server_ip in "${!SERVERS[@]}"; do
            ...
            if [[ -n "$creds" && "$creds" != "null" ]]; then
                CREDENTIALS["$server_ip"]="$creds"
                log_message "SUCCESS" "✅ Imported credentials for $server_ip from AWS"
            else
                log_message "WARNING" "⚠️  No credentials found for $server_ip in AWS Secrets"
            fi
        done
        return 0
    else
        log_message "ERROR" "❌ Failed to retrieve secret from AWS Secrets Manager"
        return 1
    fi
}
...
import_from_env_vars() {
    ...
    for server_ip in "${!SERVERS[@]}"; do
        ...
        if [[ -n "$creds" ]]; then
            CREDENTIALS["$server_ip"]="$creds"
            ((loaded_count++))
            log_message "SUCCESS" "✅ Imported credentials for $server_ip from env var"
        else
            log_message "WARNING" "⚠️  Missing environment variable: $env_var"
        fi
    done
    
    if [[ $loaded_count -gt 0 ]]; then
        log_message "SUCCESS" "✅ Imported $loaded_count credentials from environment"
        return 0
    else
        log_message "ERROR" "❌ No credentials found in environment variables"
        return 1
    fi
}
...
import_credentials() {
    ...
    log_message "INFO" "📥 Importing credentials using source: $import_source"
...
initialize_credentials() {
    log_message "INFO" "🔐 Initializing credentials using method: $AUTH_METHOD"
...
log_message() {
    ...
    case "$level" in
        ...
        "ERROR")
            ...
            ;;
        "HEADER")
            echo -e "${PURPLE}🔹 [HEADER]${NC} $message" | tee -a "$LOG_FILE"
            ;;
        *)
...
prompt_install_package() {
    ...
    echo ""
    echo -e "${YELLOW}📦 Package '$package' is not installed on server $server_ip${NC}"
    while true; do
        echo -e "${CYAN}🤔 Do you want to install '$package'? (y/N):${NC} \c"
        read -r response
        ...
        case "${response,,}" in  # Convert to lowercase
            y|yes)
                ...
                ;;
            n|no|"")
                log_message "INFO" "  ⏭️  Skipping installation of $package (user choice)"
...
install_package() {
    ...
    if [[ "$install_cmd" == "unknown" ]]; then
        log_message "ERROR" "  ❌ Cannot install $package: Unknown OS type"
        return 1
    fi
...
check_system_health() {
    ...
    log_message "HEADER" "🏥 System Health Check for $server_ip"
    ...
    if [[ -n "$cpu_load" ]]; then
        ...
        log_message "WARNING" "  ⚠️  Could not retrieve CPU load"
    fi
    ...
    if [[ -n "$memory_info" ]]; then
        log_message "INFO" "  🧠 Memory Usage:"
        echo "$memory_info" | while IFS= read -r line; do
            log_message "INFO" "    $line"
        done
    else
        log_message "WARNING" "  ⚠️  Could not retrieve memory information"
    fi
    ...
    if [[ -n "$disk_usage" ]]; then
        log_message "INFO" "  💾 Root Disk Usage: $disk_usage"
    else
        log_message "WARNING" "  ⚠️  Could not retrieve disk usage"
    fi
    ...
    if [[ -n "$root_perms" ]]; then
        log_message "INFO" "  🔐 Root Directory Permissions: $root_perms"
    else
        log_message "WARNING" "  ⚠️  Could not retrieve root permissions"
    fi
    ...
    if [[ -n "$uptime_info" ]]; then
        ...
        log_message "WARNING" "  ⚠️  Could not retrieve uptime information"
    fi
}
...
check_packages() {
    ...
    log_message "HEADER" "📦 Package Check for $server_ip (Role: $server_role)"
    ...
    if [[ "$os_type" == "unknown" ]]; then
        log_message "ERROR" "  ❌ Unknown OS type, skipping package checks"
        return 1
    fi
    ...
    if [[ -z "$required_packages" ]]; then
        log_message "WARNING" "  ⚠️  No required packages defined for role: $server_role"
        return 0
    fi
    
    log_message "INFO" "  📋 Required packages: $required_packages"
    
    # Check each package
    local packages_to_install=()
    for package in $required_packages; do
        if is_package_installed "$server_ip" "$package" "$os_type"; then
            log_message "SUCCESS" "  ✅ $package is installed"
        else
            log_message "WARNING" "  ⚠️  $package is NOT installed"
    ...
    if [[ ${#packages_to_install[@]} -gt 0 ]]; then
        log_message "INFO" "  🔧 Found missing packages: ${packages_to_install[*]}"
...
check_ports() {
    ...
    log_message "HEADER" "🔌 Port Check for $server_ip (Role: $server_role)"
    ...
    for service in "${ports_for_role[@]}"; do
        ...
        if [[ -n "$port" ]]; then
            if nc -z -w"$PORT_CHECK_TIMEOUT" "$server_ip" "$port" 2>/dev/null; then
                log_message "SUCCESS" "  ✅ $service (port $port) is open"
            else
                log_message "ERROR" "  ❌ $service (port $port) is closed or filtered"
            fi
        fi
    done
}
...
monitor_server() {
    ...
    log_message "HEADER" "🖥️  Starting monitoring for $server_ip (Role: $server_role)"
    
    # Test SSH connectivity first
    if ! test_ssh_connection "$server_ip"; then
        log_message "ERROR" "  ❌ SSH connection failed to $server_ip"
        return 1
    fi
    
    log_message "SUCCESS" "  ✅ SSH connection established to $server_ip"
    ...
    log_message "SUCCESS" "  ✅ Completed monitoring for $server_ip"
...
generate_pdf_report() {
    ...
    log_message "INFO" "📄 Checking for PDF generation tools..."
    ...
    if command -v enscript &> /dev/null && command -v ps2pdf &> /dev/null; then
        ...
        if enscript --font=Courier8 --header="Server Monitoring Report - $TIMESTAMP" \
           ...
           ps2pdf "${log_file}.ps" "$pdf_file" &>/dev/null; then
            ...
            log_message "SUCCESS" "✅ PDF report generated: $pdf_file"
        ...
        log_message "WARNING" "⚠️  PDF generation tools not available (enscript/ghostscript)"
...
print_summary() {
    ...
    log_message "HEADER" "📊 MONITORING SUMMARY"
    log_message "INFO" "  📈 Total servers monitored: $total_servers"
    log_message "INFO" "  ✅ Successful connections: $successful_connections"
    log_message "INFO" "  ❌ Failed connections: $failed_connections"
    
    if [[ $failed_connections -eq 0 ]]; then
        log_message "SUCCESS" "  🎉 All servers monitored successfully!"
    else
        log_message "WARNING" "  ⚠️  Some servers could not be monitored"
    fi
}
...
main() {
    ...
    log_message "INFO" "🚀 Starting server monitoring process..."
    log_message "INFO" "📊 Monitoring ${#SERVERS[@]} servers"
    echo "" | tee -a "$LOG_FILE"
    ...
    for server_ip in "${!SERVERS[@]}"; do
        ...
        log_message "INFO" "🔍 Processing server $server_count/${#SERVERS[@]}: $server_ip"
        
        # Monitor server (continue even if one fails)
        if ! monitor_server "$server_ip" "$server_role"; then
            log_message "WARNING" "⚠️  Monitoring failed for $server_ip, continuing with next server..."
        fi
    ...
    # Final message
    log_message "SUCCESS" "🏁 Server monitoring completed!"
...
trap 'log_message "ERROR" "❌ Script interrupted by user"; exit 130' INT TERM

# Validate that we have servers to monitor
if [[ ${#SERVERS[@]} -eq 0 ]]; then
    echo -e "${RED}❌ Error: No servers defined in SERVERS array${NC}"
    exit 1
fi
...
for server_ip in "${!SERVERS[@]}"; do
    if [[ -z "${CREDENTIALS[$server_ip]:-}" ]]; then
        echo -e "${RED}❌ Error: No credentials defined for server $server_ip${NC}"
        exit 1
    fi
done
